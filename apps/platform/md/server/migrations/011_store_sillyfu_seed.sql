...
SELECT id, 'sillyfu-standard', '傻福虾盘 标准版', 'USB-A + USB-C 双接口 | 128G', '/static/img/sillyclaw/product-front-silver.png', 29900, 500, 1, TRUE
FROM store_collections WHERE collection_key = 'sillyfu'
ON CONFLICT DO NOTHING;

-- 高速版
INSERT INTO store_products (collection_id, product_key, name_zh, description_zh, image_url, price, stock_count, sort_order, is_active)
SELECT id, 'sillyfu-highspeed', '傻福虾盘 高速版', 'USB-A + USB-C 双接口 | 256G | 高速读写', '/static/img/sillyclaw/product-black.png', 49900, 300, 2, TRUE
FROM store_collections WHERE collection_key = 'sillyfu'
ON CONFLICT DO NOTHING;

-- Pro 版
INSERT INTO store_products (collection_id, product_key, name_zh, description_zh, image_url, price, stock_count, sort_order, is_active)
SELECT id, 'sillyfu-pro', '傻福虾盘 Pro', 'USB-A + USB-C 双接口 | 512G | 极速读写', '/static/img/sillyclaw/glowing.jpg', 89900, 200, 3, TRUE
FROM store_collections WHERE collection_key = 'sillyfu'
ON CONFLICT DO NOTHING;
