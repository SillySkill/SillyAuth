# -*- coding: utf-8 -*-
"""
企微消息监听服务 + OpenClaw 推送 (修正版)
使用 HTTP API 而不是 WebSocket 推送
"""
import asyncio
import websockets
import json
...
async def forward_to_openclaw(wechat_data: dict):
    ...
    if not session_key:
        print("[OpenClaw] ⚠️  Session key 未配置，跳过推送")
        return False
    ...
    try:
        ...
        timestamp = wechat_data.get('timestamp', datetime.now().isoformat())
        ...
        if response.status_code == 200:
            ...
            print(f"[OpenClaw][{time_str}] ✅ 已推送到 session (总计: {forwarded_count})", flush=True)
            return True
        else:
            print(f"[OpenClaw] ❌ 推送失败: HTTP {response.status_code} - {response.text}", flush=True)
            return False

    except httpx.ConnectError:
        print("[OpenClaw] ⚠️  连接失败 (OpenClaw 可能未运行)", flush=True)
        return False
    except Exception as e:
        print(f"[OpenClaw] ❌ 推送异常: {type(e).__name__}: {e}", flush=True)
        return False
...
async def main():
    ...
    print("="*60, flush=True)
    print("企微回复监听服务 + OpenClaw HTTP API 推送", flush=True)
    print("="*60, flush=True)
    print(f"设备列表: {DEVICES}", flush=True)
    print(f"User ID: 33 (傻小黑)", flush=True)
    print(f"SillyMD 服务器: {WS_URL[:60]}...", flush=True)
    print(f"OpenClaw API: {OPENCLAW_API_URL}", flush=True)
    print(f"Session Key: {session_key[:20]}..." if session_key else "Session Key: 未配置", flush=True)
    print("="*60, flush=True)

    if not session_key:
        print("\n⚠️  警告: 未配置 Session Key，消息将不会被推送到 OpenClaw")
        print("\n设置方法:")
        print("  1. 命令行参数: python listen_with_openclaw_push_fixed.py <session_key>")
        print("  2. 环境变量: set OPENCLAW_SESSION_KEY=<session_key>")
        print()
